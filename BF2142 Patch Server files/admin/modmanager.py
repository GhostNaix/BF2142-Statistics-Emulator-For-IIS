...
This is a Module Manager for BattleField 2
...
class ModManager( object ):
	...
	def __init__( self, debuglog ):
		...
		self.info( 'Creating Multiplay, ModManager v%s (www.multiplay.co.uk)' % __version__ )
		...
		self.info( 'Initialising Multiplay, ModManager v%s' % __version__ )
	...
	def __setConfigFile( self ):
		...
		try:
			...
			self.__gameString = self.__bf2String
			self.__gameId = self.__bf2Id
			configFileParts = host.rcon_invoke( 'sv.configFile' ).split( '/' );
	...
	def cmdSetParam( self, ctx, cmd ):
		...
		if var.startswith( 'restricted' ):
			msg = "Failed to set param '%s' ( restricted param )\n" % cmd
			self.warn( msg )
			ctx.write( msg )
			return 0
