...
class BanManager( object ):
	...
	def validateBanMethod( self, banMethod=None, banKey=None, banAddress=None ):
		...
		if banMethod != mm_utils.BanMethod.key and banMethod != mm_utils.BanMethod.address:
	...
	def loadBans( self ):
		...
		if not mm_utils.fileexists( self.__banFileName ):
			...
			ban_re = re.compile( 'Key:\s+(\S+)\s+(\S+)' )
			for line in host.rcon_invoke( 'admin.listBannedKeys' ).split( '\n' ):
				line = line.strip()
				match = ban_re.search( line )
				if match is not None:
					cdkeyhash = self.validateBanCdKeyHash( match.group(1) )
					self.__addBan( mm_utils.BanMethod.key, None, match.group(2), None, cdkeyhash, None, None, None, None, True )
				elif line:
