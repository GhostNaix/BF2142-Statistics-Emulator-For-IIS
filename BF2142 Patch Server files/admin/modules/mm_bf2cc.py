...
class BF2CC:
	...
	def __init__( self, modManager ):
		...
		self.__cmds = {
			...
			'killplayer': { 'method': self.cmdKillPlayer, 'args': '<playerid>', 'restricted': 1, 'level': 30 },
			'switchplayer': { 'method': self.cmdSwitchPlayer, 'args': '<playerid> [1|2|3]', 'level': 20 },
			'fling': { 'method': self.cmdFling, 'args': '<playerid> <height>', 'restricted': 1, 'level': 30 },
	...
	def GetPlayerInfo( self ):
		...
		for player in pattern.findall(rawData):
			playerData[int(player[0])] = player[1:]
	...
	def cmdGetPlayerList( self, ctx, cmd ):
		...
		for p in bf2.playerManager.getPlayers():
			...
			try:
				...
				elif str(playerData[p.index][3]) == '':
					pparts.append( 'LAN - No Hash' )			#42
				else:
					pparts.append( playerData[p.index][3] )		#42
	...
	def cmdGetSnapshot( self, ctx, cmd ):
		...
		#parts.append( host.sgl_getParam( 'ticketChangePerSecond', 1, 1 ) )
		parts.append( '0' )														#12
		...
		#parts.append( host.sgl_getParam( 'ticketChangePerSecond', 2, 1 ) )
		parts.append( '0' )
		...
		# query at... For compare to player connected.
		parts.append( str( host.timer_getWallTime() ) ) 
		
		# query ReservedSlot Count
		ReservedSlots = host.rcon_invoke( 'sv.numreservedslots' )
		parts.append ( ReservedSlots )
	...
	def cmdKillPlayer( self, ctx, cmd ):
		...
		#vehin = player.getVehicle()
		#p = vehin.getOccupyingPlayers()[0]
		veh.setDamage(0)
		#host.pmgr_p_set("sgr", 0, 0)

		# bah still crashing server, tried everythiinv.
		#p.isAlive = False
		#ctx.write(str(dir(p)))
		#if veh != vehin:
		#	host.pmgr_p_set("vehicle", int(cmd), veh)

		#veh.setDamage(0)
		#veh = player.getVehicle()
		#veh.setPosition(tuple([float(-10000), float(10000), float(-10000)]))
		#veh.setDamage(0)
		#veh.killed = True
		#ctx.write(str(dir(veh)))
		...
		#try:
			#veh.getDamage()
		#	veh.setDamage(0)
		#	ctx.write('Player killed')
		#except:
		#	ctx.write('Error: Cannot kill player because of vehicle bug.')

		#ctx.write(str(veh.getDamage()))
		...
		#print str(dir(bf2.playerManager.getPlayerByIndex(int(cmd))))
		#try:

		#host.pmgr_p_set("suicide", int(cmd), 1)
		#plrs = host.pmgr_getPlayers()
		#tmp = plrs[0]

		#vehicle = player.getDefaultVehicle()
		#vehicle.setDamage(0)
		#player.setTimeToSpawn(5)

		#player.setTeam(1)

		#ctx.write(str(dir(host)))

		#ctx.write(str(dir(host.pmgr_p_get)))

		#host.pmgr_p_set("damage", int(cmd), 5)
		#vehicle = player.getVehicle()
		#vehicle = vehicle.getParent()


		#ctx.write(str(dir(tmp)))
		#except:
		#	ctx.write('Error: Could not kill player')
