...
class AdminServer(object):
	...
	def __init__( self, modManager ):
		...
		self.registerCmdHandler( __cmd_kick_player__, { 'method': self.cmdKickPlayer, 'args': '<playerid> "<reason>"', 'level': 5 } )
		self.registerCmdHandler( __cmd_ban_player__, { 'method': self.cmdBanPlayer, 'args': '<playerid> <period> "<reason>"', 'level': 10 } )
		self.registerCmdHandler( __cmd_ban_player_by__, { 'method': self.cmdBanPlayerBy, 'args': '<playerid> <bannedby> <period> "<reason>"', 'level': 10 } )
	...
	def allowMap( self, map_name ):
		...
		if host.ss_getParam('ranked'):
			...
			modDir = self.modDir()
			if self.__officialMaps.has_key( modDir ) and self.__officialMaps[modDir].has_key( map_name ):
				return True
			else:
				return False
		else:
	...
	def cmdExec( self, ctx, cmd ):
		...
		self.mm.debug( 2, "cmdExec '%s' by %s" % ( cmd, ctx.getName() ) )
		try:
			( subcmd, args ) = mm_utils.lsplit( cmd.strip(), ' ', 2 )
			subcmd_orig = subcmd
			subcmd = subcmd.lower()
	...
	def cmdKickPlayer( self, ctx, cmd ):
		"""Kick a player from the server with a message."""
		self.mm.debug( 2, "cmdKickPlayer '%s' by %s" % ( cmd, ctx.getName() ) )
	...
	def cmdBanPlayer( self, ctx, cmd ):
		"""Ban a player for a specified timer period from the server with a message."""
		self.mm.debug( 2, "cmdBanPlayer '%s' by %s" % ( cmd, ctx.getName() ) )
	...
	def cmdBanPlayerBy( self, ctx, cmd ):
		"""Ban a player for a specified timer period from the server with a message."""
